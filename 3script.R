...
## Regressione LDA
# Calcolo manuale e sicuro dei voti predetti dal modello
voti_predetti_slda <- as.vector(theta_slda %*% slda_model$coefs)
if (!is.null(slda_model$intercept)) {
  voti_predetti_slda <- voti_predetti_slda + slda_model$intercept
} else {
  voti_predetti_slda <- voti_predetti_slda + (mean(y_slda) - mean(voti_predetti_slda))
}

# --- Calcolo delle Metriche di Performance per sLDA ---
rmse_slda <- sqrt(mean((y_slda - voti_predetti_slda)^2))
r2_slda   <- 1 - (sum((y_slda - voti_predetti_slda)^2) / sum((y_slda - mean(y_slda))^2))
