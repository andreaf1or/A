...
# Struttura: 1. Setup  2. Dati  3. Preprocessing  4. Esplorativa
#            5. Bigrammi  6. sLDA  7. Regressione penalizzata
#            8. Classificazione e Regressione su DTM sparsa
# =========================================================================
...
# =========================================================================
# 6. ANALISI ESPLORATIVA
# =========================================================================

# --- 6a. Distribuzione dei voti -------------------------------------------
movies_clean %>%
  ...
  labs(
    ...
    subtitle = "Suddivisa in classi: Alto (≥7) e Basso (<7)",
    x        = "Voto Medio", y = "Numero di Film", fill = "Classe di Voto:"
  )
ggsave("hist_voti.pdf", width = 8, height = 5)

# --- 6b. Boxplot voto per genere ------------------------------------------
movies_clean %>%
  ...
  labs(
    ...
    subtitle = "La linea rossa indica la soglia Alto (≥7)",
    x = "Genere", y = "Voto Medio"
  )
ggsave("votixgenere.png", width = 9, height = 6)

# --- 6c. Top-20 parole più frequenti (barplot) ----------------------------
word_counts %>%
  slice_max(n, n = 20) %>%
...
ggsave("20parole.png", width = 8, height = 6)

# --- 6d. Wordcloud (top-100 parole) ---------------------------------------
word_counts %>%
...
# --- 6e. Top-10 parole per genere (barplot facet) -------------------------
...
movies_slda %>%
  ...
  labs(
    ...
    x = "Voto Medio Reale (TMDB)", y = NULL
  )
ggsave("plot_slda_voto_topic.pdf", width = 9, height = 5)
...
movies_slda %>%
  ...
  geom_text_repel(aes(label = topic_label), size = 3.5,
...
ggsave("plot_slda_pop_voto.pdf", width = 9, height = 6)

# --- 8d. Coefficienti sLDA (effetto netto centrato) ----------------------
...
tibble(
  ...
  labs(
    ...
    subtitle = "Effetto netto stimato rispetto alla media globale",
    x = "Scostamento stimato dal Voto Medio", y = NULL, fill = NULL
  )
ggsave("plot_slda_coef.pdf", width = 9, height = 5)


# =========================================================================
# 9. DTM (tm) – REGRESSIONE PENALIZZATA (Lasso / Ridge / Elastic Net)
# =========================================================================
...
# --- 9a. Lasso / Ridge / Elastic Net (split 75/25) -----------------------
set.seed(1234)
...
ggsave("plot_coef_lasso_reg.pdf", width = 8, height = 8)


# =========================================================================
# 10. MODELLI PREDITTIVI SULLA DTM SPARSA (freq >= 20)
# =========================================================================
...
ggsave("plot_roc_lasso.pdf", width = 7, height = 6)
...
ggsave("plot_coef_lasso_class.pdf", width = 8, height = 7)
...
ggsave("plot_rf_importance.pdf", width = 8, height = 6)
...
ggplot(data.frame(actual = y_reg_test, predicted = pred_reg_lasso),
  ...
  labs(
    ...
    x = "Vote Average reale", y = "Vote Average previsto"
  )
ggsave("plot_lasso_reg_sparse.pdf", width = 7, height = 6)
...
ggsave("plot_rf_reg.pdf", width = 7, height = 6)
